...
module.exports.announce = function (req, res) {
	...
	if (typeof par == "string") {
		...
		if (peerlimit > cfg.numwantlimit)
			peerlimit = cfg.numwantlimit;
		if (peerlimit < 0)
			peerlimit = 0;
		...
		connection.query("SELECT * FROM tracker.torrents WHERE info_hash=" + mysql.escape(par.info_hash) + " AND event != 'stopped' AND port != 0 LIMIT " + peerlimit + " ORDER BY togo ASC, last_update DESC", function (error, results, fields) {
			...
			if (response.incomplete < 0 || response.incomplete.isNaN)
...
var announceGetParameters = function (req) {
	...
	if (ret.ip == "127.0.0.1" || ret.ip == "localhost")
		ret.ip = cfg.serverIP;
	ret.numwant = req.query.numwant;
	ret.trackerid = req.query.trackerid;

	if (!ret.info_hash)
		return "f-ihm"; // False-InfoHashMissing
	if (!ret.peer_id)
		return "f-pim"; // False-PeerIdMissing
	if (!ret.port)
		return "f-pm"; // False-PortMissing

	//  if (ret.info_hash.length != 20) return "f-ihl"; // False-InfoHashLength
	if (ret.info_hash.length != 20 && ret.info_hash.length != 40)
		return "f-ihl";
	if (ret.peer_id.length != 20)
		return "f-pil"; // False-PeerIdLength
	if (ret.port > 65535)
		return "f-pr"; // False-PortRange
